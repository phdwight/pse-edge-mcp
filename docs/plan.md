...
**Purpose: protect PSE Edge.** For **prices** the server is deliberately an **EOD data service** — upstream price queries happen around market-closed times, and everything in between is served from the shared cache; intraday price freshness is a non-goal. For every other domain the protection is **deduplication, not a time gate**: one upstream hit per unique query per boundary window, at whatever hour it is first asked (scope narrowed 2026-08-07 — see the struck-through bullet below).
...
- ~~**Cache miss during open hours: refuse.**~~ **Softened to a labelled one-time fetch (decided 2026-08-07, 0.13.0).** A *cached* price is still never refetched during the session. But refusing a price nobody has ever asked for served an invariant, not a user: there is no previous value to protect, and the refusal was the only case where the server had literally nothing to say. Now a never-cached price key is fetched **once** during the session (single-flighted, throttled) and served flagged `stale: true` plus a `meta.note` stating it is **not a realtime value** — and the quote tool surfaces **only `previous_close`** (identity + the last settled price; every session-moving field, `raw_fields` included, is withheld so a delayed intraday number can never masquerade as a price). The label is derived from the entry's `fetched_at`, so every repeat that session carries it, and the first ask after the 15:00 close replaces the snapshot with the settled EOD figures. `MARKET_OPEN_NO_CACHE` is no longer raised; the error class is retained so clients that handle the code keep working. Intraday traffic to PSE Edge from price tools is bounded at one fetch per never-cached key per session.
- ~~**Uniform scope: ALL data domains follow this policy**~~ **Scope narrowed to prices (decided 2026-08-07, 0.13.0).** The full freeze — including the open-market refusal above — now applies **only to price data** (`get_stock_quote`, `get_price_history`; `data_policy: "EOD-frozen"`). Every other domain (companies, disclosures, profiles, financials, dividends, indices/summary) is **fetch-once-then-persist** (`data_policy: "daily-refresh"`): a cache miss may hit PSE Edge at *any* hour — once, single-flighted — and every repeat of the same query is served from storage until the next market close, when the first query refetches. Rationale: the original refusal made most of the tool surface unusable for six hours a day to protect against traffic the persist-on-first-hit already prevents; PSE Edge still sees at most one request per unique query per boundary window. A disclosure filed at 10 AM is now visible the first time someone asks. Immutable objects (disclosure details by `edge_no`) never refetch at all, and their first fetch is likewise no longer gated. `FreezeService.get()` takes an explicit per-read `policy`; the *default* remains `EOD-frozen`, so an unlabelled read can only over-protect PSE Edge.
...
- **Machine-to-machine: `client_credentials`, gated on a provisioning flag (decided 2026-08-01).** Headless agents — a LangGraph app, the Anthropic Messages API MCP connector — cannot open a browser, and both original grants are browser-bound. The alternative in use was an operator pasting a *personal* token into a service, which is a human's credential doing a program's job. `client_credentials` fixes that, but it interacts badly with a decision already made: **dynamic client registration is open to the internet.** So the right to use the grant must not be derivable from anything a registrant supplies — not a self-declared `grant_types` array, not a requested `token_endpoint_auth_method`, not the presence of a secret. It is authorized by `oauth_clients.client_type == 'machine'`, a column written **only** by `pse-edge-admin create-machine-client`, and the type check runs *before* any secret comparison so the endpoint cannot be used to enumerate client ids. Get this wrong and anyone on the internet can mint tokens; it is the most important test in the suite.
- **Supporting decisions for that grant.** A machine client is backed by a **service user** under the reserved, non-routable `machine.invalid` domain — chosen so the bearer path stays byte-identical for both grants (`/mcp` validates by joining `auth_tokens` to `users`, and a userless token would need a special case in the middleware, which is exactly where a revocation check goes missing later), and so quotas, usage accounting and disablement apply with no extra code. **No refresh token** for this grant: the client already holds a long-lived secret, so a refresh token would be a second credential of equal power without the rotation benefit that justifies one. **`POST /oauth/token` is rate-limited** per `client_id` and per IP — full-entropy secrets are hopeless to guess, but "the secret is strong" and "the endpoint accepts unlimited guesses" are different questions.
...
- **Observability — delivered 2026-08-01.** Structured JSON logs throughout, timestamped in both formats, with INFO on the critical paths (every upstream fetch, auth and quota refusals, the OAuth lifecycle) and a startup line stating the resolved configuration. The **nightly canary** (`canary.py`, `pse-edge-canary`) hits one real endpoint per family during closed hours and validates each against the Pydantic model the tool would build, emailing `PSE_OPERATOR_EMAIL` on failure. No metrics stack in v1 (revisit if remote usage grows).
  Three decisions taken while building it. It **bypasses the cache**: `FreezeService` would answer from a warm entry and validate yesterday's HTML, proving nothing about today's. It **still refuses to run while the market is open** and makes no request at all then — bypassing the cache is not licence to bypass invariant #1, and learning about drift a few hours later is far cheaper than being the reason PSE Edge sees mid-session traffic. And it **alerts only on failure**: a nightly "all fine" message is filtered within a week, at which point a filtered alert is worse than none because it feels like coverage.
  Each check must mirror how the *repository* builds its model, not merely parse successfully. The first live run reported a failure that turned out to be the check asserting a shape the client never returns — a canary that cries wolf gets ignored, which is the same outcome as not having one.
- **Upstream outage: serve the last close, flagged (decided 2026-08-01).** If a fetch fails because PSE Edge is unreachable and an expired cache entry exists, it is served with `stale: true` rather than discarded. Holding real data and answering with an error instead is strictly worse, and `stale` already means "real data, past its boundary" — so a client that handles the market-open case handles an outage unchanged, and `as_of` states exactly how old the answer is. With nothing cached there is genuinely no answer to give, and `EDGE_UNAVAILABLE` stands; its meaning narrows to "unreachable **and** nothing cached".
...
- **Privacy compliance (registration data) — delivered 2026-07-31.** Collecting emails makes the operator a personal-information controller under the PH Data Privacy Act (and GDPR for foreign users). All five requirements are implemented: `/privacy` policy page (public — a policy you must sign up to read is not a policy); `/account` subject-access view plus a self-service `POST /account/delete` that erases immediately with no approval step; email as the only identifying field collected; usage retention capped at **90 days**, purged automatically; breach-notification contact stated in the policy.
  ...
  4. CSRF tokens on every state-changing form — `/consent`, `/account/delete`, `/account/sessions/revoke`, and the machine-client create/revoke routes — derived from the session id rather than stored. SameSite=Lax already blocks the cross-site POST; this is defence in depth.
...
- **Backups:** scheduled `pg_dump` (daily, during closed hours) with rotation/retention in the production compose file (`compose.nas.yaml`); the named volume plus dumps cover the two unrecoverable assets — user accounts and the accumulated archive.
...
- **Phase 6 — Remote deploy (delivered):** originally `compose.prod.yaml` overlay + `Caddyfile` + `docs/deploy.md` (Caddy TLS with automatic ACME). *Revised 2026-08-12:* the Caddy path was retired; production is the standalone `compose.nas.yaml`, whose `tunnel` profile adds `cloudflared` — Cloudflare terminates TLS at its edge, so no inbound ports, ACME or port-forwarding anywhere. `db` still publishes no port. Auth is **on by default in production** — anonymous access to a public deployment should not be something you opt out of. Daily `pg_dump` at 02:00 Asia/Manila (after the close, away from the boundary refetch) with rotation that runs only after a *successful* dump, plus a daily usage-retention purge.
  Decisions taken while building it:
  ...
  4. **Per-worker state is documented, not hidden.** Quota windows, parse memo and token cache are per process, so N workers means a user's ceiling is up to N× nominal. `compose.nas.yaml` defaults to 1 (NAS cores are scarce); `docs/deploy.md` says to scale limits with workers.
