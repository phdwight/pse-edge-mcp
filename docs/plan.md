...
- ~~**Cache miss during open hours: refuse.**~~ **Softened to a labelled one-time fetch (decided 2026-08-07, 0.13.0).** A *cached* price is still never refetched during the session. But refusing a price nobody has ever asked for served an invariant, not a user: there is no previous value to protect, and the refusal was the only case where the server had literally nothing to say. Now a never-cached price key is fetched **once** during the session (single-flighted, throttled) and served flagged `stale: true` plus a `meta.note` stating it is **not a realtime value** — the label is derived from the entry's `fetched_at`, so every repeat that session carries it, and the first ask after the 15:00 close replaces the snapshot with the settled EOD figures. `MARKET_OPEN_NO_CACHE` is no longer raised; the error class is retained so clients that handle the code keep working. Intraday traffic to PSE Edge from price tools is bounded at one fetch per never-cached key per session.
