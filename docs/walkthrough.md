...
| Bearer token (refresh) | same generator | `pse_` + 43 chars | **SHA-256 only** | 30 days (`PSE_REFRESH_TTL_DAYS`); **2 days until the family first refreshes** (`PSE_REFRESH_UNUSED_TTL_DAYS`) — an abandoned sign-in falls off the account page in days |
...
| `PSE_REFRESH_UNUSED_TTL_DAYS` | `2` | Refresh lifetime until a family first rotates; abandoned sign-ins expire early |
