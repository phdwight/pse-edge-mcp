...
|---|---|---|---|---|
| Bearer token (access) | `"pse_" + secrets.token_urlsafe(32)` | `pse_` + 43 chars | **SHA-256 only** | 15 min (`PSE_ACCESS_TTL_MIN`) |
| Bearer token (refresh) | same generator | `pse_` + 43 chars | **SHA-256 only** | 24 h (`PSE_REFRESH_TTL_HOURS`), **single-use** — every rotation mints a new one and revokes the old pair; `PSE_REFRESH_UNUSED_TTL_HOURS` (24) caps a family that never rotated |
